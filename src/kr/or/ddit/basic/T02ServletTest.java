...
/*	
 	...
 	2. 서블릿 컨테이너는 web.xml에 설정된 url패턴 정보를 확인하여 어느 서블릿을 통해 처리할지를 검색한다.
 	 (서블릿이 컨테이너에 로딩이 안된 경우에는 먼저 생성하여 로딩 처리함. init() 메서드 호출한다.)
 	3. 서블릿 컨테이너는 요청을 처리할 개별 스레드를 생성하여 해당 서블릿 객체의 service() 메서드를 호출한다.
 	 (HttpServletRequest 및 HttpServletReponse 객체를 생성하여 파라미터로 넘겨준다.)
 	...
 	6. 컨테이너로부터 서블릿이 제거되는 경우에는 먼저 destroy()메서드가 호출된다.
 ...
 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 응답메시지의 컨테트 타입정보 설정
//		resp.setContentType("text/plain"); ///???로 나온다
//		resp.setContentType("text/plain; charset=UTF-8");
		resp.setCharacterEncoding("UTF-8"); ///이걸함으로서 ???로 나오던것들이 한글로 잘나온다
